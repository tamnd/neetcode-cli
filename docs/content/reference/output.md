...
describes what users get. Pick a format with `-o`, or let nc choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
nc <command> -o table   # aligned columns for reading
nc <command> -o jsonl   # one JSON object per line, for piping
nc <command> -o json    # a single JSON array
nc <command> -o csv     # spreadsheet friendly
nc <command> -o tsv     # tab-separated
nc <command> -o url     # just the URL column
nc <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
nc <command> --fields id,title,url
```
...
```bash
nc <command>            # a table, because this is a terminal
nc <command> | wc -l    # JSONL, because this is a pipe
```
